...
struct RLPTestcase {
    const char *data;
    rlp_kind_e expectedKind;
    uint64_t expectedLen;
    uint64_t expectedDataOffset;
    uint64_t expectedConsumed;
};

TEST(RLP, RLPDecodingKind) {
    vector<RLPTestcase> rlp_vec {
        {"00", RLP_KIND_BYTE, 0, 0, 1},
        {"01", RLP_KIND_BYTE, 0, 0, 1},
        {"7F", RLP_KIND_BYTE, 0, 0, 1},

        {"80", RLP_KIND_STRING, 0, 1, 1},
        {"B7", RLP_KIND_STRING, 55, 1, 56},
        {"B90400", RLP_KIND_STRING, 1024, 3, 1027},

        {"C0", RLP_KIND_LIST, 0, 1, 1},
        {"C8", RLP_KIND_LIST, 8, 1, 9},
        {"F7", RLP_KIND_LIST, 55, 1, 56},
        {"F90400", RLP_KIND_LIST, 1024, 3, 1027}
    };

    rlp_t rlp;
    for (const auto& testcase : rlp_vec) {
        uint8_t buffer[2000] = {0};
        parseHexString(buffer, sizeof(buffer), testcase.data);
        parser_context_t ctx = {.buffer = buffer,
                                .bufferLen = sizeof(buffer),
                                .offset = 0,
                                .tx_obj = NULL};
        const parser_error_t err = rlp_read(&ctx, &rlp);

        EXPECT_THAT(err, testing::Eq(parser_ok));
        EXPECT_THAT(rlp.kind, testing::Eq(testcase.expectedKind));
        EXPECT_THAT(rlp.rlpLen, testing::Eq(testcase.expectedLen));
        EXPECT_THAT(ctx.offset, testing::Eq(testcase.expectedConsumed));
        EXPECT_THAT(rlp.ptr - ctx.buffer, testing::Eq(testcase.expectedDataOffset));
    }
}
